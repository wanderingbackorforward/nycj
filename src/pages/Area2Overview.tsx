...
const LEVEL_COLOR: Record<string, string> = { alarm: "#e65100", warning: "#d4a050", normal: "#2e7d32", caution: "#1565c0" };
  ...
  useEffect(() => {
    ...
    try {
      ...
      alertBarInst.current.setOption({
        backgroundColor: "transparent",
        tooltip: { trigger: "axis", backgroundColor: "rgba(15,21,37,0.95)", borderColor: "#1a2640", textStyle: { color: "#c8d6e5", fontSize: 12 } },
        legend: { top: 4, textStyle: { color: "#7a8ba8", fontSize: 10 } },
        grid: { left: 50, right: 20, top: 30, bottom: 60 },
        xAxis: { type: "category", data: items.map(i => i.item || ""), axisLabel: { color: "#5a6d8a", fontSize: 10, rotate: 35 }, axisLine: { lineStyle: { color: "#1a2845" } } },
    ...
    <div className="page-area2-overview">
      ...
      {headline && (
        <div style={{
          ...
          border: "1px solid " + (overallLevel === "alarm" ? "#5a1a1a" : overallLevel === "warning" ? "#5a3a1a" : "#1a3a4a"),
          borderLeft: "4px solid " + LEVEL_COLOR[overallLevel] || "#1565c0",
          borderRadius: 4, padding: "10px 14px", marginBottom: 16
        }}>
          <span style={{ color: LEVEL_COLOR[overallLevel] || "#5a8aaa", fontSize: 13, fontWeight: 600 }}>
      ...
      {cards.length > 0 && (
        <section style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
          {cards.map((card, i) => {
            const levelColor = LEVEL_COLOR[card.level] || "#5a8aaa";
            return (
              <div key={i} style={{
      ...
      {monSummary?.alert_summary && (
        <section style={{ marginBottom: 16, background: "#0f1525", border: "1px solid #1a2640", borderRadius: 6, padding: 12 }}>
          ...
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            <div style={{ background: "#1a1010", border: "1px solid #5a1a1a", borderRadius: 4, padding: "8px 14px", flex: "1 1 100px" }}>
            ...
            <div style={{ background: "#111a2e", border: "1px solid #1a2845", borderRadius: 4, padding: "8px 14px", flex: "1 1 100px" }}>
              <span style={{ fontSize: 11, color: "#5a8aaa" }}>总读数</span>
              <span style={{ fontSize: 22, fontWeight: 700, color: "#5a8aaa", marginLeft: 8 }}>{(monSummary.total_readings || 0).toLocaleString()}</span>
            </div>
          </div>
        </section>
      )}
      ...
      {mileage?.rings && mileage.rings.length > 0 && (
        <section style={{ marginBottom: 16, background: "#0f1525", border: "1px solid #1a2640", borderRadius: 6, padding: 12 }}>
          ...
          <p style={{ color: "#4a5a6e", fontSize: 11, marginBottom: 8 }}>区间里程：DK30+594 ~ DK30+543</p>
        ...
        <section style={{ marginBottom: 16, background: "#0f1525", border: "1px solid #5a3a1a", borderLeft: "4px solid #e65100", borderRadius: 4, padding: 12 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {nearbyRisks.map((r, i) => {
              ...
              const levelColor = levelCn === "高" ? "#e65100" : "#5a8aaa";
        ...
        <section style={{ marginBottom: 16, background: "#0f1525", border: "1px solid #1a2640", borderRadius: 6, padding: 12 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {overview.findings.map((f, i) => {
              ...
              const lvlColor = LEVEL_COLOR[String(fRec.level || "")] || "#1565c0";
              return (
