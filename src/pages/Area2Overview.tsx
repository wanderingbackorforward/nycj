...
const LEVEL_COLOR: Record<string, string> = { alarm: "var(--color-danger)", warning: "var(--color-warning)", normal: "var(--color-success)", caution: "var(--color-accent)" };
...
export default function Area2Overview() {
    ...
    try {
      ...
      gaugeInst.current.setOption({
        ...
        series: [{
          ...
          axisLine: { lineStyle: { width: 18, color: [[pct / 100, "var(--color-accent)"], [1, "var(--color-panel-border)"]] } },
          axisTick: { show: false }, splitLine: { show: false },
          axisLabel: { color: "var(--color-text-dim)", fontSize: 10, distance: 20, formatter: "{value}环" },
          pointer: { length: "65%", width: 6, itemStyle: { color: "var(--color-danger)" } },
          detail: { valueAnimation: true, formatter: "当前\n{value}环", color: "var(--color-text-primary)", fontSize: 18, offsetCenter: [0, "55%"], lineHeight: 22 },
          data: [{ value: curr }],
        }],
      }, true);
    } catch { /* silent */ }
  ...
  useEffect(() => {
    ...
    try {
      ...
      tlInst.current.setOption({
        ...
        xAxis: { type: "category", data: dates, axisLabel: { color: "var(--color-text-dim)", fontSize: 10 }, axisLine: { lineStyle: { color: "var(--color-panel-border)" } } },
        yAxis: { type: "value", name: "环号", nameTextStyle: { color: "var(--color-text-dim)" }, axisLabel: { color: "var(--color-text-dim)", fontSize: 11 }, splitLine: { lineStyle: { color: "var(--color-bg-grid)" } } },
        series: [{
          type: "line", data: ringNums, smooth: true,
          itemStyle: { color: "var(--color-accent)" }, lineStyle: { width: 2 },
  ...
  useEffect(() => {
    ...
    try {
      ...
      alertBarInst.current.setOption({
        backgroundColor: "transparent",
        tooltip: { trigger: "axis", backgroundColor: "rgba(15,21,37,0.95)", borderColor: "var(--color-panel-border)", textStyle: { color: "var(--color-text-primary)", fontSize: 12 } },
        legend: { top: 4, textStyle: { color: "var(--color-text-secondary)", fontSize: 10 } },
        grid: { left: 50, right: 20, top: 30, bottom: 60 },
        xAxis: { type: "category", data: items.map(i => i.item || ""), axisLabel: { color: "var(--color-text-dim)", fontSize: 10, rotate: 35 }, axisLine: { lineStyle: { color: "var(--color-panel-border)" } } },
        yAxis: { type: "value", axisLabel: { color: "var(--color-text-dim)", fontSize: 10 }, splitLine: { lineStyle: { color: "var(--color-bg-grid)" } } },
        series: [
          { name: "报警", type: "bar", stack: "total", data: items.map(i => i.alarm || 0), itemStyle: { color: "var(--color-danger)" }, barWidth: 20 },
          { name: "预警", type: "bar", stack: "total", data: items.map(i => i.warning || 0), itemStyle: { color: "var(--color-warning)" } },
          { name: "正常", type: "bar", stack: "total", data: items.map(i => i.normal || 0), itemStyle: { color: "var(--color-success)" } },
        ],
      }, true);
    } catch { /* silent */ }
  ...
  return (
    <div className="page-area2-overview">
      ...
      {headline && (
        <div style={{
          background: overallLevel === "alarm" ? "var(--color-danger-bg)" : overallLevel === "warning" ? "#2a1a0a" : "#101a20",
          border: "1px solid " + (overallLevel === "alarm" ? "var(--color-danger-border)" : overallLevel === "warning" ? "var(--color-warning-border)" : "var(--color-accent-muted)"),
          borderLeft: "4px solid " + LEVEL_COLOR[overallLevel] || "var(--color-accent)",
          borderRadius: 4, padding: "10px 14px", marginBottom: 16
        }}>
          <span style={{ color: LEVEL_COLOR[overallLevel] || "var(--color-text-dim)", fontSize: 13, fontWeight: 600 }}>
      ...
      {cards.length > 0 && (
        <section style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
          {cards.map((card, i) => {
            const levelColor = LEVEL_COLOR[card.level] || "var(--color-text-dim)";
            return (
              <div key={i} style={{
                flex: "1 1 180px", background: "var(--color-panel)", border: "1px solid var(--color-panel-border)",
                borderLeft: "4px solid " + levelColor, borderRadius: 4, padding: "12px 14px"
              }}>
                <div style={{ fontSize: 11, color: "var(--color-text-muted)", marginBottom: 4 }}>{card.title}</div>
                <div style={{ fontSize: 22, fontWeight: 700, color: levelColor }}>{card.value}</div>
                <div style={{ fontSize: 10, color: "var(--color-text-dim)", marginTop: 2 }}>{card.subtitle || ""}</div>
              </div>
            );
          })}
        </section>
      ...
      <section style={{ display: "flex", gap: 16, marginBottom: 16 }}>
        <div style={{ flex: 1, background: "var(--color-panel)", border: "1px solid var(--color-panel-border)", borderRadius: 6, padding: 12 }}>
          <h4 style={{ color: "var(--color-text-muted)", fontSize: 13, marginBottom: 4 }}>环号进度</h4>
          <div ref={gaugeRef} style={{ height: 220 }} />
        </div>
        <div style={{ flex: 2, background: "var(--color-panel)", border: "1px solid var(--color-panel-border)", borderRadius: 6, padding: 12 }}>
          <h4 style={{ color: "var(--color-text-muted)", fontSize: 13, marginBottom: 4 }}>环号时间轴（含里程标注）</h4>
        ...
        <section style={{ display: "flex", gap: 16, marginBottom: 16 }}>
          {analytics?.monitoring?.by_item && (
            <div style={{ flex: 1.5, background: "var(--color-panel)", border: "1px solid var(--color-panel-border)", borderRadius: 6, padding: 12 }}>
              <h4 style={{ color: "var(--color-text-muted)", fontSize: 13, marginBottom: 4 }}>监测项目报警分布</h4>
          ...
          <div style={{ flex: 1, background: "var(--color-panel)", border: "1px solid var(--color-panel-border)", borderRadius: 6, padding: 12 }}>
            <h4 style={{ color: "var(--color-text-muted)", fontSize: 13, marginBottom: 8 }}>掘进参数分组</h4>
                ...
                return (
                  <div key={i}>
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, marginBottom: 2 }}>
                      <span style={{ color: "var(--color-text-secondary)" }}>{g.group_cn || g.group_code}</span>
                      <span style={{ color: "var(--color-text-dim)" }}>{(g.sample_count || 0).toLocaleString()}条 / {g.ring_count || 0}环</span>
                    </div>
                    <div style={{ height: 5, background: "var(--color-panel-border)", borderRadius: 2, overflow: "hidden" }}>
                      <div style={{ height: "100%", width: barPct + "%", background: "linear-gradient(90deg, var(--color-accent-dim), var(--color-accent))", borderRadius: 2 }} />
                    </div>
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10 }}>
                      <span style={{ color: "var(--color-accent)", fontWeight: 600 }}>{(g.sample_count || 0).toLocaleString()}条</span>
                      {g.exceed_count != null && g.exceed_count > 0 ? (
                        <span style={{ color: "var(--color-danger)" }}>{g.exceed_count}条超限</span>
                      ) : (
                        <span style={{ color: "var(--color-text-dim)" }}>正常</span>
                      )}
                    </div>
                  </div>
                );
      ...
      {monSummary?.alert_summary && (
        <section style={{ marginBottom: 16, background: "var(--color-panel)", border: "1px solid var(--color-panel-border)", borderRadius: 6, padding: 12 }}>
          <h4 style={{ color: "var(--color-text-muted)", fontSize: 13, marginBottom: 8 }}>监测数据摘要</h4>
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            <div style={{ background: "var(--color-danger-bg)", border: "1px solid var(--color-danger-border)", borderRadius: 4, padding: "8px 14px", flex: "1 1 100px" }}>
              <span style={{ fontSize: 11, color: "var(--color-danger)" }}>报警</span>
              <span style={{ fontSize: 22, fontWeight: 700, color: "var(--color-danger)", marginLeft: 8 }}>{(monSummary.alert_summary.alarm || 0).toLocaleString()}</span>
            </div>
            <div style={{ background: "#1a1a10", border: "1px solid #5a4a2a", borderRadius: 4, padding: "8px 14px", flex: "1 1 100px" }}>
              ...
              <span style={{ fontSize: 22, fontWeight: 700, color: "var(--color-warning)", marginLeft: 8 }}>{(monSummary.alert_summary.warning || 0).toLocaleString()}</span>
            </div>
            <div style={{ background: "var(--color-success-bg)", border: "1px solid var(--color-success-border)", borderRadius: 4, padding: "8px 14px", flex: "1 1 100px" }}>
              <span style={{ fontSize: 11, color: "var(--color-success)" }}>正常</span>
              <span style={{ fontSize: 22, fontWeight: 700, color: "var(--color-success)", marginLeft: 8 }}>{(monSummary.alert_summary.normal || 0).toLocaleString()}</span>
            </div>
            <div style={{ background: "var(--color-bg-hover)", border: "1px solid var(--color-panel-border)", borderRadius: 4, padding: "8px 14px", flex: "1 1 100px" }}>
              <span style={{ fontSize: 11, color: "var(--color-text-dim)" }}>总读数</span>
              <span style={{ fontSize: 22, fontWeight: 700, color: "var(--color-text-dim)", marginLeft: 8 }}>{(monSummary.total_readings || 0).toLocaleString()}</span>
            </div>
          </div>
        </section>
      )}
      ...
      {mileage?.rings && mileage.rings.length > 0 && (
        <section style={{ marginBottom: 16, background: "var(--color-panel)", border: "1px solid var(--color-panel-border)", borderRadius: 6, padding: 12 }}>
          <h4 style={{ color: "var(--color-text-muted)", fontSize: 13, marginBottom: 4 }}>环号-里程映射（{mileage.rings.length}环）</h4>
          <p style={{ color: "var(--color-text-dim)", fontSize: 11, marginBottom: 8 }}>区间里程：DK30+594 ~ DK30+543</p>
            ...
            <table className="db-table" style={{ width: "100%", fontSize: 11 }}>
              ...
              <tbody>
                {mileage.rings.slice(0, 5).map((r, i) => (
                  <tr key={i}><td className="mono">{r.ring_no}</td><td className="mono">{r.chainage}</td><td style={{ fontSize: 10, color: "var(--color-text-dim)" }}>{r.mileage}</td></tr>
                ))}
                {mileage.rings.length > 5 && <tr><td colSpan={3} style={{ textAlign: "center", color: "var(--color-text-dim)", fontSize: 10 }}>... 共{mileage.rings.length}环</td></tr>}
              </tbody>
            </table>
          </div>
        </section>
      ...
      {nearbyRisks.length > 0 && (
        <section style={{ marginBottom: 16, background: "var(--color-panel)", border: "1px solid var(--color-warning-border)", borderLeft: "4px solid var(--color-danger)", borderRadius: 4, padding: 12 }}>
          <h4 style={{ color: "var(--color-danger)", fontSize: 13, marginBottom: 4 }}>附近风险源（{nearbyRisks.length}个）</h4>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {nearbyRisks.map((r, i) => {
              ...
              const levelColor = levelCn === "高" ? "var(--color-danger)" : "var(--color-text-dim)";
              const desc = [cat, ringInfo ? "环" + ringInfo : "", mileInfo ? "· " + mileInfo : ""].filter(Boolean).join(" · ");
              return (
                <div key={i} style={{ background: "var(--color-warning-bg)", border: "1px solid #3a2a1a", borderRadius: 4, padding: "8px 12px", minWidth: 160 }}>
                  <div style={{ fontSize: 11, fontWeight: 600, color: "var(--color-warning)" }}>{r.name}</div>
      ...
      {overview?.findings && overview.findings.length > 0 && (
        <section style={{ marginBottom: 16, background: "var(--color-panel)", border: "1px solid var(--color-panel-border)", borderRadius: 6, padding: 12 }}>
          <h4 style={{ color: "var(--color-text-muted)", fontSize: 13, marginBottom: 6 }}>关键发现</h4>
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {overview.findings.map((f, i) => {
              ...
              const lvlColor = LEVEL_COLOR[String(fRec.level || "")] || "var(--color-accent)";
              return (
                <div key={i} style={{ fontSize: 12, color: "var(--color-text-secondary)", display: "flex", alignItems: "flex-start", gap: 8 }}>
      ...
      {overview?.dataGaps && overview.dataGaps.length > 0 && (
        <section style={{ marginBottom: 0, background: "var(--color-panel)", border: "1px solid var(--color-panel-border)", borderRadius: 6, padding: 12 }}>
          <h4 style={{ color: "var(--color-text-muted)", fontSize: 13, marginBottom: 6 }}>数据缺口</h4>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            {overview.dataGaps.map((g, i) => {
              ...
              const statusColor = status === "P0" ? "var(--color-danger)" : status === "P1" ? "var(--color-warning)" : "var(--color-text-dim)";
              return (
                <div key={i} style={{ background: "var(--color-bg-hover)", border: "1px solid var(--color-panel-border)", borderRadius: 4, padding: "6px 12px" }}>
                  <span style={{ fontSize: 10, fontWeight: 600, color: "var(--color-warning)" }}>{field}</span>
