...
export default function Area1PointAnalysis() {
  ...
  const trendOption = useMemo(() => {
    ...
    return {
      ...
      xAxis: { type: "category", data: dates, axisLabel: { color: "#5a6d8a", fontSize: 10, rotate: 30 }, axisLine: { lineStyle: { color: "#1a2845" } } },
      yAxis: [
    ...
    <div className="page-area1-analysis">
      ...
      {diagnosis?.findings && diagnosis.findings.length > 0 ? (
        <section style={{marginBottom:14}}>
          ...
          <div style={{background:"#0f1525", border:"1px solid #1a2640", borderLeft:"4px solid " + (diagnosis.findings.some(f => f.level === "critical") ? "#e65100" : "#1565c0"), borderRadius:4, padding:14}}>
            {diagnosis.findings.map((f, i) => (
              <div key={i} style={{display:"flex", alignItems:"flex-start", gap:8, marginBottom:6}}>
                <span style={{background: f.level === "critical" ? "#e65100" : "#1565c0", color:"#fff", borderRadius:2, padding:"1px 6px", fontSize:10, fontWeight:600, whiteSpace:"nowrap", marginTop:1}}>
                  {f.level === "critical" ? "!!" : "i"}
                </span>
            ...
            {diagnosis.recommendation && (
              <div style={{fontSize:12, color:"#5a8aaa", borderTop:"1px solid #1a2640", paddingTop:8, marginTop:4, lineHeight:1.6}}>
            ...
            {diagnosis.siblings_comparison && diagnosis.siblings_comparison.filter(s => s.latest_cumulative != null).length > 0 && (
              <div style={{marginTop:10, borderTop:"1px solid #1a2640", paddingTop:8}}>
                <div style={{fontSize:11, color:"#5a8aaa", marginBottom:6}}>同期同类点对比（累计变化）</div>
                <div style={{display:"flex", gap:8, flexWrap:"wrap"}}>
                  {diagnosis.siblings_comparison.filter(s => s.latest_cumulative != null).slice(0, 12).map((s, i) => (
                    <div key={i} style={{background:"#111a2e", border:"1px solid #1a2845", borderRadius:3, padding:"6px 10px", textAlign:"center", minWidth:70}}>
            ...
            {diagnosis.readings && diagnosis.readings.length > 0 && (
              <div style={{marginTop:10, borderTop:"1px solid #1a2640", paddingTop:8}}>
                <div style={{fontSize:11, color:"#5a8aaa", marginBottom:4}}>原始数据（共{diagnosis.reading_count}条，最近10条）</div>
        ...
        <section style={{marginBottom:14}}>
          ...
          <div style={{display:"flex", flexDirection:"column", gap:8}}>
            {analysis.findings.map((f, i) => (
              <div key={i} style={{background:"#0f1525", border:"1px solid #1a2640", borderLeft:"3px solid #1565c0", borderRadius:4, padding:"10px 12px"}}>
          ...
          <div style={{display:"flex", gap:10, overflowX:"auto", paddingBottom:6}}>
            {evItems.map((e, i) => (
              <div key={i} style={{minWidth:170, background:"#0f1525", border:"1px solid #1a2640", borderRadius:4, padding:10, flexShrink:0}}>
                ...
                <div className="mono" style={{fontSize:11, color:"#8a9bb5", wordBreak:"break-all", marginBottom:2}}>{String(e.file_name || "-")}</div>
