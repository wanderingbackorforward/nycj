...
async def operation(ring_no: Optional[int] = Query(default=None, description="环号，不传则默认最新环")):
    return get_operation_diagnosis(ring_no)

@router.get("/diagnosis/slurry-grouting")
async def slurry_grouting(ring_no: Optional[int] = Query(default=None, description="环号，不传则默认最新环")):
