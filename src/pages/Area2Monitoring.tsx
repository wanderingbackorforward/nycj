...
export default function Area2Monitoring() {
  ...
  useEffect(() => {
    ...
    barInst.current.setOption({
      ...
      xAxis: { type: "category", data: items.map(i => (i.monitoring_item || "").replace("竖向位移","竖向").replace("水平位移","水平")), axisLabel: { color: "#5a6d8a", fontSize: 10, rotate: 35 }, axisLine: { lineStyle: { color: "#1a2845" } } },
  ...
  useEffect(() => {
    ...
    pieInst.current.setOption({
      ...
      series: [{ type: "pie", radius: ["40%", "65%"], center: ["50%", "45%"], data, label: { color: "#8a9bb5", fontSize: 11 }, itemStyle: { borderColor: "#0a0e1a", borderWidth: 2 } }],
    }, true);
  ...
  useEffect(() => {
    ...
    radarInst.current.setOption({
      ...
      series: [{
        type: "radar",
        data: [{ name: "2工区监测", value: [confScore, 75, 0, 70, 60], areaStyle: { color: "rgba(255,140,66,0.1)" }, lineStyle: { color: "#ff8c42" }, itemStyle: { color: "#ff8c42" } }],
      }],
    }, true);
    ...
    <div className="page-area2-monitoring">
      ...
      <section style={{marginBottom:14}}>
        ...
        <div style={{display:"flex", gap:8, flexWrap:"wrap"}}>
          <div style={{background:"#1a1010", border:"1px solid #5a1a1a", borderRadius:4, padding:"8px 14px"}}>
          ...
          <div style={{background:"#111a2e", border:"1px solid #1a2845", borderRadius:4, padding:"8px 14px"}}>
            <span style={{fontSize:11, color:"#5a8aaa"}}>待确认</span>
            <span style={{fontSize:18, fontWeight:700, color:"#5a8aaa", marginLeft:8}}>{raw?.alert_summary ? (raw.alert_summary as Record<string,number>).unknown?.toLocaleString() || "0" : "0"}</span>
          </div>
        </div>
      </section>
    </div>
