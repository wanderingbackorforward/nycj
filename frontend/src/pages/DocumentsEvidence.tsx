  ...
  useEffect(() => {
    ...
    gaugeInst.current.setOption({
      ...
      series: [{
        ...
        min: 0, max: total,
        axisLine: { lineStyle: { width: 14, color: [[pct/100, "#e65100"], [1, "#1a2640"]] } },
        pointer: { length: "65%", width: 5, itemStyle: { color: "#e65100" } },
        axisTick: { show: false }, splitLine: { show: false },
        axisLabel: { show: false },
        detail: { valueAnimation: true, formatter: "总计\n{value}份", color: "#c8d6e5", fontSize: 18, offsetCenter: [0, "55%"], lineHeight: 22 },
        data: [{ value: total }],
      }],
    }, true);
        ...
        <div style={{display:"flex", gap:16, alignItems:"center"}}>
          <div style={{flex:1}}>
            <div style={{display:"flex", justifyContent:"space-between", marginBottom:4}}>
              <span style={{fontSize:12, color:"#98aec9"}}>已登记</span>
              <span style={{fontSize:12, color:"#00d4ff", fontWeight:600}}>{cadDocs.length} 份</span>
            </div>
            <div style={{height:8, background:"#1a2640", borderRadius:4, overflow:"hidden"}}>
            ...
            <div style={{display:"flex", justifyContent:"space-between", marginTop:4, marginBottom:8}}>
              <span style={{fontSize:12, color:"#98aec9"}}>空间配准</span>
          ...
          <div style={{flex:1, borderLeft:"1px solid #1a2640", paddingLeft:16}}>
            <p style={{fontSize:12, color:"#98aec9", lineHeight:1.6}}>
        ...
        <div style={{display:"flex", gap:12, flexWrap:"wrap"}}>
          {Object.entries(cats).map(([cat, docs]) => (
            <div key={cat} style={{flex:"1 1 250px", background:"#0f1525", border:"1px solid #1a2640", borderRadius:6, padding:12}}>
              <h4 style={{color:"#5a6d8a", fontSize:12, marginBottom:8}}>{DOC_CATEGORY_CN[cat] || cat}（{docs.length} 份）</h4>
