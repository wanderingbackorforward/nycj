...
export default function Area2Tunneling() {
  ...
  return (
    <div>
      <h2 className="text-base font-semibold mb-1" style={{ color: "var(--color-text-primary)" }}>2工区掘进参数</h2>
      ...
      <div className="card card-warning mb-4">
        <h3 className="text-sm font-semibold mb-2" style={{ color: "var(--color-warning)" }}>当前结论</h3>
        <p className="text-xs leading-relaxed mb-2" style={{ color: "var(--color-text-secondary)" }}>
          五个参数分组共{totalParams.toLocaleString()}条记录，其中
          <strong style={{ color: "var(--color-danger)" }}>{totalExceed.toLocaleString()}条超出统计阈值（P05/P95）</strong>，占比{exceedPct}%。
          {topExceedGroup && <>超限最多的分组为<strong style={{ color: "var(--color-warning)" }}>{topExceedGroup.group_name_cn}</strong>（{topExceedGroup.exceed_count?.toLocaleString()}条）。</>}
        </p>
      ...
      {/* 参数分组卡片 */}
      <h3 className="text-sm font-semibold mb-2" style={{ color: "var(--color-text-muted)" }}>关键证据：五组超限分布</h3>
        ...
        {groups.map((g, i) => {
          ...
          return (
            <div key={i} onClick={() => setSelectedGroup(String(g.group_code ?? ""))} className="flex-1 min-w-[170px] rounded p-2.5 cursor-pointer transition-all" style={{
              background: selectedGroup === g.group_code ? "var(--color-bg-hover)" : "var(--color-panel)",
              border: isTop ? "1px solid var(--color-danger-border)" : selectedGroup === g.group_code ? "1px solid var(--color-accent-muted)" : "1px solid var(--color-panel-border)",
              borderLeft: isTop ? "4px solid var(--color-danger)" : "4px solid var(--color-accent-muted)",
            }}>
              <div className="text-[11px] font-semibold mb-1" style={{ color: "var(--color-text-secondary)" }}>{g.group_name_cn || g.group_code}</div>
              <div className="flex justify-between items-baseline">
                <span className="text-lg font-bold" style={{ color: "var(--color-text-primary)" }}>{(g.parameter_count || 0).toLocaleString()}</span>
                <span className="text-[10px]" style={{ color: "var(--color-text-dim)" }}>条参数</span>
              </div>
              <div className="h-1 rounded-sm overflow-hidden mt-1.5" style={{ background: "var(--color-panel-border)" }}>
                <div className="h-full rounded-sm" style={{ width: Math.min(100, Number(pct) * 5) + "%", background: isTop ? "var(--color-danger)" : "var(--color-warning)" }} />
              </div>
              <div className="flex justify-between mt-1 text-[10px]">
                <span style={{ color: (g.exceed_count || 0) > 0 ? "var(--color-danger)" : "var(--color-success)" }}>超限 {(g.exceed_count || 0).toLocaleString()} 条</span>
                <span style={{ color: "var(--color-text-dim)" }}>{pct}%</span>
              </div>
            </div>
          );
        })}
      ...
      <div className="card mb-4">
        ...
        <div ref={chartRef} style={{ height: 300 }}>
          {!paramsLoading && params.length === 0 && (
            <div className="flex items-center justify-center h-full text-sm" style={{ color: "var(--color-text-dim)" }}>选择参数分组后显示趋势图</div>
          )}
        </div>
      </div>
      ...
      <div className="card card-accent mb-4">
        <h3 className="text-sm font-semibold mb-2" style={{ color: "var(--color-accent)" }}>建议动作</h3>
        <div className="flex flex-col gap-1.5">
          {[1,2,3].map(i => (
            <div key={i} className="flex items-start gap-2 text-xs" style={{ color: "var(--color-text-secondary)" }}>
              <span style={{ color: "var(--color-accent)", fontWeight: 700 }}>{i}.</span>
      ...
      <div className="card mb-0">
        <h3 className="text-sm font-semibold mb-2" style={{ color: "var(--color-text-muted)" }}>数据缺口</h3>
        <div className="flex gap-2.5 flex-wrap">
          {[
            ...
            <div key={i} className="flex-1 min-w-[200px] rounded p-2.5" style={{ background: "var(--color-warning-bg)", border: "1px solid var(--color-warning-border)" }}>
              <div className="text-[11px] font-semibold mb-1" style={{ color: "var(--color-warning)" }}>{g.cat}</div>
              <div className="text-[11px] mb-1" style={{ color: "var(--color-text-dim)" }}>{g.detail}</div>
              <div className="text-[10px]" style={{ color: "var(--color-text-muted)" }}>{'→ ' + g.action}</div>
            </div>
          ))}
        </div>
      </div>
