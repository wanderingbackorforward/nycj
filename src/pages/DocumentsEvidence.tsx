...
export default function DocumentsEvidence() {
  ...
  useEffect(() => {
    ...
    pieInst.current.setOption({
      ...
      legend: { orient: "vertical", right: 10, top: "center", textStyle: { color: "var(--color-text-secondary)", fontSize: 11 } },
      series: [{ type: "pie", radius: ["45%", "70%"], center: ["38%", "50%"], data: Object.entries(catCounts).map(([cat, count]) => ({ name: DOC_CATEGORY_CN[cat] || cat, value: count })), label: { color: "var(--color-text-secondary)", fontSize: 10 }, itemStyle: { borderColor: "var(--color-bg-deep)", borderWidth: 2 } }],
    }, true);
  ...
  useEffect(() => {
    ...
    gaugeInst.current.setOption({
      ...
      series: [{
        ...
        min: 0, max: total,
        axisLine: { lineStyle: { width: 14, color: [[pct/100, "var(--color-danger)"], [1, "var(--color-panel-border)"]] } },
        pointer: { length: "65%", width: 5, itemStyle: { color: "var(--color-danger)" } },
        axisTick: { show: false }, splitLine: { show: false },
        axisLabel: { show: false },
        detail: { valueAnimation: true, formatter: "总计\n{value}份", color: "var(--color-text-primary)", fontSize: 18, offsetCenter: [0, "55%"], lineHeight: 22 },
        data: [{ value: total }],
      }],
    }, true);
  ...
  useEffect(() => {
    ...
    tlInst.current.setOption({
      backgroundColor: "transparent",
      tooltip: { trigger: "item", backgroundColor: "rgba(15,21,37,0.95)", borderColor: "var(--color-panel-border)", textStyle: { color: "var(--color-text-primary)", fontSize: 12 }, formatter: (p: { value: string[]; data: { docName: string } }) => p.value[0] + "<br/>" + p.value[1] + "<br/>" + p.data.docName },
      grid: { left: 140, right: 20, top: 10, bottom: 24 },
      xAxis: { type: "category", data: [...new Set(dates)], axisLabel: { color: "var(--color-text-dim)", fontSize: 9, rotate: 40 }, axisLine: { lineStyle: { color: "var(--color-panel-border)" } }, name: "日期", nameTextStyle: { color: "var(--color-text-dim)" } },
      yAxis: { type: "category", data: catList.map(c => DOC_CATEGORY_CN[c] || c), axisLabel: { color: "var(--color-text-secondary)", fontSize: 10 }, axisLine: { lineStyle: { color: "var(--color-panel-border)" } } },
      series: [{ type: "scatter", data: scatterData, symbolSize: 12, itemStyle: { color: "var(--color-accent)", borderColor: "var(--color-bg-deep)", borderWidth: 1 } }],
    }, true);
    ...
    <div className="page-documents">
      ...
      <section style={{display:"flex", gap:16, marginBottom:16}}>
        <div style={{flex:1, background:"var(--color-panel)", border:"1px solid var(--color-panel-border)", borderRadius:6, padding:12}}>
          <h4 style={{color:"var(--color-text-muted)", fontSize:13, marginBottom:4}}>文档类型分布</h4>
          <div ref={pieRef} style={{height:260}} />
        </div>
        <div style={{flex:1, background:"var(--color-panel)", border:"1px solid var(--color-panel-border)", borderRadius:6, padding:12}}>
          <h4 style={{color:"var(--color-text-muted)", fontSize:13, marginBottom:4}}>证据总量</h4>
      ...
      <section style={{marginBottom:16, background:"var(--color-panel)", border:"1px solid var(--color-panel-border)", borderRadius:6, padding:12}}>
        <h4 style={{color:"var(--color-text-muted)", fontSize:13, marginBottom:4}}>文档时间线分布（按类别）</h4>
        <div ref={tlRef} style={{height:240}} />
      </section>

      {/* ======== Row 3: CAD 配准状态 ======== */}
      <section style={{marginBottom:16, background:"var(--color-panel)", border:"1px solid var(--color-panel-border)", borderRadius:6, padding:14}}>
        <h3 style={{color:"var(--color-text-muted)", fontSize:14, marginBottom:8}}>CAD 图纸配准状态</h3>
        <div style={{display:"flex", gap:16, alignItems:"center"}}>
          <div style={{flex:1}}>
            <div style={{display:"flex", justifyContent:"space-between", marginBottom:4}}>
              <span style={{fontSize:12, color:"var(--color-text-secondary)"}}>已登记</span>
              <span style={{fontSize:12, color:"var(--color-accent)", fontWeight:600}}>{cadDocs.length} 份</span>
            </div>
            <div style={{height:8, background:"var(--color-panel-border)", borderRadius:4, overflow:"hidden"}}>
              <div style={{height:"100%", width:"100%", background:"linear-gradient(90deg, var(--color-accent-dim), var(--color-accent))", borderRadius:4}} />
            </div>
            <div style={{display:"flex", justifyContent:"space-between", marginTop:4, marginBottom:8}}>
              <span style={{fontSize:12, color:"var(--color-text-secondary)"}}>空间配准</span>
              <span style={{fontSize:12, color:"var(--color-warning)", fontWeight:600}}>0 份</span>
            </div>
            <div style={{height:8, background:"var(--color-panel-border)", borderRadius:4, overflow:"hidden"}}>
              <div style={{height:"100%", width:"0%", background:"#5a4a2a", borderRadius:4}} />
            </div>
          </div>
          <div style={{flex:1, borderLeft:"1px solid var(--color-panel-border)", paddingLeft:16}}>
            <p style={{fontSize:12, color:"var(--color-text-secondary)", lineHeight:1.6}}>
              <strong style={{color:"var(--color-warning)"}}>CAD 图纸已登记，待空间配准。</strong><br/>
      ...
      {/* ======== Row 4: 文档清点摘要 ======== */}
      <section>
        <h3 style={{color:"var(--color-text-muted)", fontSize:14, marginBottom:8}}>文档清单（{a2Docs.length} 份）</h3>
        <div style={{display:"flex", gap:12, flexWrap:"wrap"}}>
          {Object.entries(cats).map(([cat, docs]) => (
            <div key={cat} style={{flex:"1 1 250px", background:"var(--color-panel)", border:"1px solid var(--color-panel-border)", borderRadius:6, padding:12}}>
              <h4 style={{color:"var(--color-text-dim)", fontSize:12, marginBottom:8}}>{DOC_CATEGORY_CN[cat] || cat}（{docs.length} 份）</h4>
              <table className="db-table" style={{width:"100%"}}>
                ...
                <tbody>
                  ...
                  {docs.length > 5 && <tr><td colSpan={2} style={{fontSize:10, color:"var(--color-text-dim)", textAlign:"center"}}>... 还有 {docs.length - 5} 份</td></tr>}
                </tbody>
              </table>
            </div>
          ))}
