...
export default function SystemStatus() {
  ...
  useEffect(() => {
    ...
    barInst.current.setOption({
      ...
      yAxis: { type: "category", data: allKeys, axisLabel: { color: "#8a9bb5", fontSize: 10, width: 110, overflow: "truncate" }, axisLine: { lineStyle: { color: "#1a2845" } } },
      series: [
        ...
        { name: "2工区", type: "bar", data: allKeys.map(k => a2Tables[k] || 0), itemStyle: { color: "#ff8c42", borderRadius: [0,3,3,0] }, barWidth: 14, barGap: "30%", label: { show: true, position: "right", color: "#5a6d8a", fontSize: 10, formatter: (p: { value: number }) => p.value > 0 ? (p.value >= 1000 ? (p.value/1000).toFixed(0)+"k" : String(p.value)) : "" } },
      ],
    }, true);
  ...
  useEffect(() => {
    ...
    unknownBarInst.current.setOption({
      ...
      xAxis: { type: "value", axisLabel: { color: "#5a6d8a", fontSize: 10, formatter: (v: number) => v >= 1000 ? (v/1000).toFixed(0)+"k" : String(v) }, splitLine: { lineStyle: { color: "#121e36" } } },
      yAxis: { type: "category", data: reasons.map(r => r.unknown_reason_cn || r.unknown_reason), axisLabel: { color: "#8a9bb5", fontSize: 10, width: 140, overflow: "truncate" }, axisLine: { lineStyle: { color: "#1a2845" } } },
      series: [{
        type: "bar", data: reasons.map(r => r.count),
        barWidth: 16, itemStyle: { borderRadius: [0,3,3,0], color: new echarts.graphic.LinearGradient(0,0,1,0,[{offset:0,color:"#d4a050"},{offset:1,color:"#ff8c42"}]) },
    ...
    <div className="page-system-status">
      ...
      {gapSummary && (
        <section style={{marginBottom:16}}>
          ...
          <div style={{display:"flex", gap:12}}>
            <div style={{flex:1, background:"#1a1010", border:"1px solid #5a1a1a", borderRadius:6, padding:14, textAlign:"center"}}>
            ...
            <div style={{flex:1, background:"#0f1525", border:"1px solid #1a2640", borderRadius:6, padding:14, textAlign:"center"}}>
              ...
              <div style={{fontSize:12, color:"#5a8aaa", marginTop:2}}>后端已支持</div>
            </div>
            <div style={{flex:1, background:"#0f1525", border:"1px solid #1a2640", borderRadius:6, padding:14, textAlign:"center"}}>
              <div style={{fontSize:28, fontWeight:700, color:"#ff8c42"}}>{gapSummary.needs_human_input}</div>
            ...
            {gnGaps.gaps.map(gap => (
              <div key={gap.id} style={{background:"#0f1525", border:"1px solid " + (gap.priority === "P1" ? "#5a1a1a" : "#5a3a1a"), borderLeft:"4px solid " + (gap.priority === "P1" ? "#e65100" : "#d4a050"), borderRadius:4, padding:"12px 14px"}}>
                <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6}}>
                  <span style={{fontSize:13, fontWeight:600, color:"#e0e8f0"}}>{gap.title}</span>
                ...
                <div style={{fontSize:12, color:"#8a9bb5", lineHeight:1.5, marginBottom:4}}>{gap.description}</div>
                {gap.impact && <div style={{fontSize:11, color:"#d47070", marginBottom:4}}>{"影响: " + gap.impact}</div>}
                {gap.backend_capability && <div style={{fontSize:11, color:"#5a8aaa", marginBottom:4}}>{"后端能力: " + gap.backend_capability}</div>}
                  ...
                  <div>
                    ...
                    <div style={{display:"flex", gap:8}}>
                      {gnQuality.confidence_distribution.map(c => (
                        <div key={c.parse_confidence} style={{flex:1, background:"#111a2e", borderRadius:3, padding:"6px 8px", textAlign:"center"}}>
                          <div style={{fontSize:16, fontWeight:700, color: c.parse_confidence === "high" ? "#4caf50" : "#ff8c42"}}>{c.count.toLocaleString()}</div>
                  ...
                  <div>
                    ...
                    <div style={{display:"flex", gap:6}}>
                      {gnQuality.review_level_distribution.map(r => (
                        <div key={r.review_level} style={{flex:1, background:"#111a2e", borderRadius:3, padding:"6px 6px", textAlign:"center"}}>
      ...
      {gnThresholds?.thresholds && gnThresholds.thresholds.length > 0 && (
        <section style={{marginBottom:16}}>
          ...
          <p style={{color:"#4a5a6e", fontSize:11, marginBottom:8}}>{gnThresholds.note || "若此表为空，则使用 Excel 设计限值作为唯一判断依据。"}</p>
      ...
      <section style={{marginBottom:16}}>
        ...
        {gnReviews?.reviews && gnReviews.reviews.length > 0 ? (
          ...
          <div style={{background:"#0f1525", border:"1px solid #1a2640", borderRadius:6, padding:14, textAlign:"center"}}>
            <div style={{fontSize:24, color:"#4a5a6e", marginBottom:4}}>暂无复核记录</div>
        ...
        <section style={{marginBottom:16}}>
          ...
          {gnSensorPatterns.unmatched_columns && gnSensorPatterns.unmatched_columns.length > 0 && (
            <div style={{marginTop:12, background:"#0f1525", border:"1px solid #1a2640", borderRadius:6, padding:12, overflowX:"auto"}}>
              <div style={{fontSize:11, color:"#5a8aaa", marginBottom:6}}>未识别列名清单（前20条，共{gnSensorPatterns.unmatched_columns.length}条）</div>
              <table className="db-table" style={{width:"100%"}}>
      ...
      {gnCoordsNeeded && (
        <section style={{marginBottom:16}}>
          ...
          <div style={{display:"flex", gap:16}}>
            ...
            <div style={{flex:2, background:"#0f1525", border:"1px solid #1a2640", borderRadius:6, padding:14}}>
              <div style={{fontSize:11, color:"#5a8aaa", marginBottom:4}}>按监测对象分类</div>
              <div style={{display:"flex", gap:6, flexWrap:"wrap"}}>
                {(() => {
                  ...
                  return Object.entries(cats).sort((a,b) => b[1] - a[1]).slice(0, 8).map(([k, v]) => (
                    <span key={k} style={{background:"#111a2e", border:"1px solid #1a2845", borderRadius:3, padding:"3px 8px", fontSize:10, color:"#8a9bb5"}}>{k}: {v}</span>
                  ));
                })()}
              </div>
            </div>
        ...
        <section style={{marginBottom:16, background:"#0f1525", border:"1px solid #1a2640", borderRadius:6, padding:12}}>
          ...
          <div style={{display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(200px, 1fr))", gap:10}}>
            {[
              ...
              { label:"风险源", value:"7个已登记", detail:"从周报+CAD提取", color:"#ff8c42" },
              ...
              <div key={i} style={{background:"#111a2e", border:"1px solid #1a2845", borderRadius:4, padding:"10px 12px"}}>
